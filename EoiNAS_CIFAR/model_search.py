...
class Network(nn.Module):
    ...
    def forward(self, input, temperature):
        ...
        weights_reduce_np = weights_reduce.data.cpu().numpy()
        weights_normal_np = weights_normal.data.cpu().numpy()
        indexes_reduce = []
        indexes_normal = []
        for i in range(14):
            idx = np.argmax(weights_reduce_np[i])
            indexes_reduce.append(idx)
        for i in range(14):
            idx = np.argmax(weights_normal_np[i])
            indexes_normal.append(idx)

        # print(self.alphas_normal)
        # print(id(self.alphas_normal))
        # print(self.alphas_normal.grad)

        return logits, indexes_normal, indexes_reduce
